...
# Publishers
pub_bb = rospy.Publisher(topicBBoxOut, Boundingboxes , queue_size=0)
...
def callback_imageMultiClass(det_image3d):    
    ...
    det_to_3d(imgClass,imgConfidence,crop,det_image3d.header )

# Convert to image to bounding boxes. 
def callback_imageOneClass(det_image2D):  
    ...
    det_to_3d(imgClass,imgConfidence,crop,det_image2D.header )


def det_to_3d(imgClass,imgConfidence,crop, header ):
    ...
    for className in classNumberAndName.keys():
        ...
        bwImg = imgConfidenceClass>threshold
        ...
        for slicer in slicers:
            ...
            tmpBB.prob = np.clip((prob-expectedMinValue)/(expectedMaxValue-expectedMinValue),0.0,1.0)
            
            #print "prob", prob, "Normalized", tmpNormalized , "NormalizedClipped", tmpBB.prob 
            tmpBB.objectType = int(classNumber)
            tmpBB.objectName = className
            
            bounding_boxes.boundingboxes.append(tmpBB)
    pub_bb.publish(bounding_boxes)
...
if topicDetImageInSingleClass == '':
    ...
    rospy.Subscriber(topicDetImageInMultiClass, ImageDetections, callback_imageMultiClass, queue_size=1)
else:
    rospy.Subscriber(topicDetImageInSingleClass, ImageDetections, callback_imageOneClass, queue_size=1)
...
#    print 'image_det_to_3d_stereo (', nodeName, ') is publishing topic: ', topicBBoxOut
    print ''
    if topicDetImageInMultiClass == '':
        print 'det_image2bb (', nodeName, ') is subscriping to topics: ', topicDetImageInSingleClass
    else:
        print 'det_image2bb (', nodeName, ') is subscriping to topics: ', topicDetImageInSingleClass
    print 'det_image2bb (', nodeName, ') is publishing topic: ', topicBBoxOut    
    rospy.spin()
